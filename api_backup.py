...
def global_choropleth(year: int = Query(..., ge=2025),
                     scenario: str = "BAU"):
    """Generate global choropleth data with country-level hydrogen production values"""
    if scenario not in SCENARIOS:
        raise HTTPException(400, "Unknown scenario")
    ...
    for region in REGIONS:
        # Get regional hydrogen production
        regional_forecast = forecast(region, year, year, scenario)
        regional_production = regional_forecast["yearly"][0]
        
        # Distribute production among countries in the region
        countries = region_to_countries.get(region, [])
        if not countries:
            continue
            
        # Create distribution weights (some countries produce more than others)
        rng = random.Random(hash(f"{region}_{scenario}_{year}"))
        ...
        for i, (iso_code, country_name) in enumerate(countries):
            ...
            country_production = regional_production * country_weight
            
            # Add some country-specific variation
            country_rng = random.Random(hash(f"{iso_code}_{scenario}_{year}"))
    ...
    return {
        ...
        "description": f"Global hydrogen production choropleth for {year} under {scenario} scenario"
    }
